...
export default function RegistrarNuevoPage() {
  const { Contratos, HabeasData, Cesaciones,Promociones, Retail} = useGraphServices();
  const {rows, loading, error, state, pageSize, handleSubmit, pageIndex, hasNext, sorts, setField, setEstado, estado, setRange, setPageSize, nextPage, reloadAll,  toggleSort, range, setSearch, search, loadFirstPage, errors, searchRegister} = useContratos(Contratos,);
