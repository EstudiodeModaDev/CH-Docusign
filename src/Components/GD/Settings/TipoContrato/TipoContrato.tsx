...
import { useTipoContrato, } from "../../../../Funcionalidades/Desplegables";
import type { maestro } from "../../../../models/Desplegables";
import { notify } from '../../../../utils/notify';

export const TipoContratoManager: React.FC = () => {
    const { Maestro, } = useCoreGraphServices();
    const { items, add, editItem, reload, remove} = useTipoContrato(Maestro);
                    ...
                    <>
                        <section className="emp-form">
                            <div className="emp-field">
                                <label className="emp-label" htmlFor="empresaNombre">Tipo de contrato</label>
                                <input id="empresaNombre" type="text" className="emp-input" placeholder="Tipo de contrato" value={state?.T_x00ed_tulo1} onChange={(e) => setState({...state, T_x00ed_tulo1: e.target.value.toLocaleUpperCase()})}/>
                            </div>
                            { isEditing &&
                                <div className="emp-actions">
                                    ...
                                    <button type="button" className="emp-btn emp-btn--ok" onClick={async () => {
                                                                                            console.table(state)
                                                                                            if(editItem){
                                                                                                await editItem({Title: "Tipo de contrato", T_x00ed_tulo1: state?.T_x00ed_tulo1}, state!.Id ?? "", );
                                                                                                notify.auto("Se ha editado el tipo de contrato con éxito")
                                                                                                setIsAdding(false)
                                                                                                setIsEditing(false)
                                                                                                reload()
                                                                                            }
                            ...
                            { isAdding &&
                                <div className="emp-actions">
                                    ...
                                    <button type="button" className="emp-btn emp-btn--ok" onClick={async () => {
                                                                                                    ...
                                                                                                    notify.auto("Se ha agregado con éxito el tipo de contrato");
                                                                                                    setIsEditing(false)
                                                                                                    setIsAdding(false)
                                                                                                    } catch (e: any) {
                                                                                                    console.error(e);
