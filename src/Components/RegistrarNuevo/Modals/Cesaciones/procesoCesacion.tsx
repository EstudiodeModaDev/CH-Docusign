...
export const CesacionSteps: React.FC<Props> = ({titulo, selectedCesacion, onClose}) => {
  const { PasosCesacion, DetallesPasosCesacion: DetallesSvc, ColaboradoresDH, ColaboradoresEDM, ColaboradoresDenim, ColaboradoresVisual} = useGraphServices();
  ...
  const handleUploadClick = async (detalle: DetallesPasosPromocion) => {const idDetalle = detalle.Id ?? ""; const file = files[idDetalle];
    ...
    const servicioColaboradores = empresa === "dh retail" ? ColaboradoresDH : empresa === "movimiento" ? ColaboradoresVisual : empresa === "denim head" ? ColaboradoresDenim : ColaboradoresEDM;
              ...
              <div className="step-card__body">
                ...
                {!isCompleted && requiereEvidencia &&
                  <button type="button" className="btn btn-xs" disabled={isCompleted} onClick={async() =>{
                                                                                                ...
                                                                                                const carpeta = `${selectedCesacion.Title}`
