...
export const TipoVacanteManager: React.FC = () => {
    const { TipoVacante, } = useGraphServices();
    const { items, add, editItem, reload, remove} = useTipoVacante(TipoVacante);
                    ...
                    <>
                        <section className="emp-form">
                            <div className="emp-field">
                                <label className="emp-label" htmlFor="empresaNombre">Tipo de vacante</label>
                                <input id="empresaNombre" type="text" className="emp-input" placeholder="Tipo de vacante" value={state?.Title} onChange={(e) => setState({...state, Title: e.target.value})}/>
                            </div>
                            { isEditing &&
