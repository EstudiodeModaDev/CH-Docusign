...
export function useCesaciones(CesacionesSvc: CesacionesService) {
  ...
  const validate = () => {
    ...
    if(!state.Celular) e.Celular = "Ingrese el numero de celular"
    if(!state.Correoelectronico) e.Correoelectronico = "Ingrese el correo electronico"
    if(!state.FechaIngreso) e.FechaIngreso = "Seleccione la fecha de ingreso"
    if(!state.FechaIngresoCesacion) e.FechaIngresoCesacion = "Seleccione la fecha de ingreso cesaciones"
    if(!state.FechaLimiteDocumentos) e.FechaLimiteDocumentos = "Seleccione la fecha limite documentos"
    if(!state.FechaSalidaCesacion) e.FechaSalidaCesacion = "Seleccione la fecha salida cesación"
    if(!state.Jefedezona) e.Jefedezona = "Seleccione un jefe de zona"
    if(!state.Nombre) e.Nombre = "Seleccione el nombre del cesado"
    if(!state.Temporal) e.Temporal = "Ingrese la temporal"
    if(!state.Tienda) e.Tienda = "Ingrese la tienda"
    if(!state.Title) e.Title = "Ingrese el celular del seleccionado"
    if(!state.Empresaalaquepertenece) e.Empresaalaquepertenece = "Ingrese el celular del seleccionado"
