...
import { includesCesacionSearch } from "../utils/cesacionSearch";
import { compareCesaciones } from "../utils/cesacionSort";
import { buildCesacionesReportFilter, buildCesacionesServerFilter } from "../utils/cesacionFilters";
...
export function useCesacionesTable(CesacionesSvc: CesacionesService, username?: string) {
  ...
  const loadBase = React.useCallback(async () => {
    ...
    try {
      const opts: GetAllOpts = buildCesacionesServerFilter(estado, range);
      const { items } = await CesacionesSvc.getAll(opts);
      setBaseRows(items ?? []);
      setPageIndex(1);
    } catch (e: any) {
  ...
  }, [CesacionesSvc, estado, range, username]);
  ...
  React.useEffect(() => {
    ...
    if (debouncedSearch.trim()) {
      data = data.filter(r => includesCesacionSearch(r, debouncedSearch));
    }

    if (sorts.length) {
      data = [...data].sort((a, b) => {
        for (const s of sorts) {
          const result = compareCesaciones(a, b, s.field, s.dir);
  ...
  const loadToReport = React.useCallback(async (from: string, to: string, enviadoPor?: string, cargo?: string, empresa?: string, ciudad?: string) => {
      ...
      try {
        const { items, } = await CesacionesSvc.getAll( buildCesacionesReportFilter(from, to, enviadoPor, cargo, empresa, ciudad));
        setRows(items);
      } catch (e: any) {
        setRows([]);
      } finally {
