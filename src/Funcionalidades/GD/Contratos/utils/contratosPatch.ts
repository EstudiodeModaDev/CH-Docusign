...
const fields: (keyof Novedad)[] = [
  "Title", "AUTONOM_x00cd_A_x0020_",  "Auxilio_x0020_de_x0020_rodamient0", "Auxilioderodamientosiono", "BARRIO_x0020_",
...
const booleanFields: (keyof Novedad)[] = [
  "Pertenecealmodelo",  "Ajustesalario", "Auxilio_x0020_de_x0020_rodamient", "Practicante", "Aprendiz",
]
...
function booleanValue(original: string): boolean{
  if(!original) return false

  const booleanOptions = ["si", "aprobado"]

  return booleanOptions.includes(original.trim().toLocaleLowerCase())
}
...
export function buildContratosPatch(original: Novedad, next: Novedad) {
  ...
  for (const k of booleanFields) {
    if (!areFieldValuesEqual(String(k), original[k], next[k], "date")) patch[k] = booleanValue(String(next[k]));
  }
