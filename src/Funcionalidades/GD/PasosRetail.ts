...
export function useDetallesPasosRetail(DetallesSvc: DetallesPasosRetail, selected?: string) {
  ...
  const handleCreateAllSteps = async (pasos: PasosProceso[], retailId: string, cargoNegocio: string) => {
    ...
    try {
      ...
      for (const p of pasos) {
        ...
        const aplica = await shouldActivate("RETAIL", idPaso, cargoNegocio, graph);
