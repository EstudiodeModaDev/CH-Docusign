...
import type { Colaboradores, UserOption } from '../../models/Commons';
...
const cache: Record<CacheKey, { data: Colaboradores[] | null; promise: Promise<Colaboradores[]> | null }> = {};
...
function mapGraphUser(u: any, i: number): Colaboradores {
...
function mapSPRowToWorker(r: any): Colaboradores {
  const f = r?.fields ?? r ?? {};
  return {
    id: String(r.ID ?? r.Id ?? f.ID ?? f.Id ?? ''),
    displayName: String(f.Nombre ?? f.DisplayName ?? f.Title ?? r.Nombre ?? r.DisplayName ?? r.Title ?? '—'),
...
async function fetchUsersFromSharePoint(opts: Options): Promise<Colaboradores[]> {
...
export function useWorkers(options: Options = {}) {
  ...
  const [workers, setWorkers] = React.useState<Colaboradores[]>([]);
  ...
  // === helpers ===
  const mapWorkersToOptions = React.useCallback((list: Colaboradores[]): UserOption[] => {
    return (list ?? [])
      .map((u) => ({
    ...
    try {
      ...
      if (!cache[key]!.promise) {
        ...
        cache[key]!.promise = (async () => {
          ...
          // ---------- 3) MERGE + DEDUP ----------
          const map = new Map<string, Colaboradores>();

          const put = (w: Colaboradores) => {
