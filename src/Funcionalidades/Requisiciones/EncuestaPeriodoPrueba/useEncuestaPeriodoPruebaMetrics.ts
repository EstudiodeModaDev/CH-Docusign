...
import type { EncuestaSatisfaccionMesRow, EncuestaSatisfaccionResumen } from "../EncuestaSatisfaccion/useEncuestaSatisfaccionMetrics";
import { ExcelProbatory } from "../../../Services/Requisiciones/Encuestas/probatory.excel.service";
import type { periodo_prueba } from "../../../models/Requisiciones/probatory";


const MONTHS = ["Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic"];

function isAprobada(r: periodo_prueba): boolean {
  return r.had_candidate_achive_probatory_goals.toLocaleLowerCase().trim() === "si, deseo continuar acompañando su proceso de desarrollo"
}

function toResumen(responses: periodo_prueba[]): EncuestaSatisfaccionResumen {
...
function toPorMes(responses: periodo_prueba[]): EncuestaSatisfaccionMesRow[] {
...
export function useEncuestaPeriodoPruebaMetrics() {
  ...
  const [responses, setResponses] = React.useState<periodo_prueba[]>([]);
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      setError(e?.message ?? "No fue posible cargar la encuesta de periodo de prueba.");
      setResponses([]);
    } finally {
      setLoading(false);
    }
