...
export class AnsRequisicionService {
  ...
  // ---------- mapping ----------
  private toModel(item: any): ansRequisicion {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
  ...
  // ---------- CRUD ----------
  async create(record: Omit<ansRequisicion, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<ansRequisicion, 'ID'>>) {
    await this.ensureIds()
  ...
  async getAll(opts?: GetAllOpts): Promise<ansRequisicion[]> {
    await this.ensureIds()
