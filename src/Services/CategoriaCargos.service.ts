...
export class CategoriaCargosService {
  ...
  constructor(
    ...
    listName = 'Cargos - CategoriaCargos'     
  ) {
    this.graph = graph;
  ...
  // ---------- mapping ----------
  private toModel(item: any): CargoCategoria {
    const f = item?.fields ?? {};
    const mapped = {
        Id: String(item?.id ?? ''),
        Title: f.Title,
  ...
  // ---------- CRUD ----------
  async create(record: Omit<CargoCategoria, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<CargoCategoria, 'ID'>>) {
    await this.ensureIds()
