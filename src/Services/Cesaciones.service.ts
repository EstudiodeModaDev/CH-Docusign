...
export class CesacionesService {
  ...
  private toModel(item: any): Cesacion {
    const f = item?.fields ?? {};
    return {
  ...
  // ---------- CRUD ----------
  async create(record: Omit<Cesacion, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<Cesacion, 'ID'>>) {
        await this.ensureIds();
  ...
  async getAll(opts?: GetAllOpts): Promise<PageResult<Cesacion>> {
    await this.ensureIds();
  ...
  private async fetchPage(url: string, isAbsolute = false): Promise<PageResult<Cesacion>> {
