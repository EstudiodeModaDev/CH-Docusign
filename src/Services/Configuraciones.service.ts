...
export class ConfiguracionesService {
  ...
  // ---------- mapping ----------
  private toModel(item: any): configuraciones {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        Title: f.Title,
  ...
  // ---------- CRUD ----------
  async create(record: Omit<configuraciones, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<configuraciones, 'ID'>>) {
    await this.ensureIds()
  ...
  async getAll(opts?: GetAllOpts): Promise<configuraciones[]> {
    await this.ensureIds()
