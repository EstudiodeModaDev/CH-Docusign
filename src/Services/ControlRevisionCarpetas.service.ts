...
export class ControlRevisionCarpetasService {
  ...
  constructor(
    ...
    listName = 'Carpetas - ControlRevisionCarpetas'     
  ) {
    this.graph = graph;
  ...
  // ---------- mapping ----------
  private toModel(item: any): ControlRevisionCarpetas {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        Title: f.Title,
        AprobadoPor: f.AprobadoPor,
        CantidadDevoluciones: f.CantidadDevoluciones,
        Cedula: f.Cedula,
        CorreoAprobadoPor: f.CorreoAprobadoPor,
        CorreoDevueltoPor: f.CorreoDevueltoPor,
        CorreoEnviadoRevisionPor: f.CorreoEnviadoRevisionPor,
        DevueltoPor: f.DevueltoPor,
        Empresa: f.Empresa,
        EnviadoRevisionPor: f.EnviadoRevisionPor,
        Estado: f.Estado,
        FechaAprobacion: f.FechaAprobacion,
        FechaDevolucion: f.FechaDevolucion,
        FechaEnvioRevision: f.FechaEnvioRevision,
        FechaUltimaAccion: f.FechaUltimaAccion,
        FolderName: f.FolderName,
        FolderPath: f.FolderPath,
        MotivoUltimaDevolucion: f.MotivoUltimaDevolucion,
        NombreColaborador: f.NombreColaborador,
        UltimaAccion: f.UltimaAccion,
        UltimoActor: f.UltimoActor
    };
  }

  // ---------- CRUD ----------
  async create(record: Partial<Omit<ControlRevisionCarpetas, 'Id'>>) {
  ...
  async update(id: string, changed: Partial<Omit<ControlRevisionCarpetas, 'Id'>>) {
    await this.ensureIds();
  ...
  async getAll(opts?: GetAllOpts): Promise<ControlRevisionCarpetas[]> {
    await this.ensureIds();
