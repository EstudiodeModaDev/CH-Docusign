...
export class EnviosService {
  ...
  private toModel(item: any): Envio {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        Cedula: f.Cedula,
        Compa_x00f1_ia: f.Compa_x00f1_ia,
        CorreoReceptor: f.CorreoReceptor,
        Datos: f.Datos,
        EnviadoPor: f.EnviadoPor,
        Estado: f.Estado,
        Fechadeenvio: f.Fechadeenvio,
        Fuente: f.Fuente,
        ID_Novedad: f.ID_Novedad,
        IdSobre: f.IdSobre,
        Receptor: f.Receptor,
  ...
  async update(id: string, changed: Partial<Omit<Envio, 'ID'>>) {
        await this.ensureIds();
  ...
  async getAll(opts?: GetAllOpts): Promise<PageResult<Envio>> {
    await this.ensureIds();
  ...
  async getByNextLink(nextLink: string): Promise<PageResult<Envio>> {
    return this.fetchPage(nextLink, /*isAbsolute*/ true);
  }

  private async fetchPage(url: string, isAbsolute = false): Promise<PageResult<Envio>> {
