...
export class HabeasDataService {
  ...
  private toModel(item: any): HabeasData {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        AbreviacionTipoDoc: f.AbreviacionTipoDoc,
        Ciudad: f.Ciudad,
        Fechaenlaquesereporta: f.Fechaenlaquesereporta,
        Informacionreportadapor: f.Informacionreportadapor,
  ...
  // ---------- CRUD ----------
  async create(record: Omit<HabeasData, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<HabeasData, 'ID'>>) {
        await this.ensureIds();
  ...
  async getAll(opts?: GetAllOpts): Promise<PageResult<HabeasData>> {
    await this.ensureIds();
  ...
  async getByNextLink(nextLink: string): Promise<PageResult<HabeasData>> {
    return this.fetchPage(nextLink, /*isAbsolute*/ true);
  }

  private async fetchPage(url: string, isAbsolute = false): Promise<PageResult<HabeasData>> {
