...
export class HistorialRevisionCarpetasService {
  ...
  // ---------- mapping ----------
  private toModel(item: any): HistorialRevisionCarpetas {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        Title: f.Title,
        ...
        Cedula: f.Cedula,
        ControlRevisionId: f.ControlRevisionId,
        CorreoRealizadoPor: f.CorreoRealizadoPor,
        EstadoAnterior: f.EstadoAnterior,
        EstadoResultante: f.EstadoResultante,
  ...
  async update(id: string, changed: Partial<Omit<HistorialRevisionCarpetas, 'Id'>>) {
    await this.ensureIds();
  ...
  async getAll(opts?: GetAllOpts): Promise<HistorialRevisionCarpetas[]> {
    await this.ensureIds();
