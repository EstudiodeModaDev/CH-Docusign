...
export class pasoRestriccionProcesoService {
  ...
  // ---------- mapping ----------
  private toModel(item: any): PasoRestriccion {
    const f = item?.fields ?? {};
    const mapped: PasoRestriccion = {
        Id: String(item?.id ?? ''),
        Activo: f.Activo,
        CargoNombre: f.CargoNombre,
        Proceso: f.Proceso,
        TipoRegla: f.TipoRegla,
        Title: f.Title
    };
    return mapped
  }
  ...
  // ---------- CRUD ----------
  async create(record: Omit<PasoRestriccion, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<PasoRestriccion, 'ID'>>) {
    await this.ensureIds()
  ...
  async getAll(opts?: GetAllOpts): Promise<PasoRestriccion[]> {
    await this.ensureIds()
