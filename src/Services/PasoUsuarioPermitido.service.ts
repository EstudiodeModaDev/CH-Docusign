...
import type { pasoUsuarioPermitido } from "../models/Pasos";
...
export class PasosRetailService {
  ...
  // ---------- mapping ----------
  private toModel(item: any): pasoUsuarioPermitido {
    const f = item?.fields ?? {};
    const mapped: pasoUsuarioPermitido = {
        Id: String(item?.id ?? ''),
        Title: f.Title,
        Activo: f.Activo,
        correoUsuario: f.correoUsuario,
        Proceso: f.Proceso,
    };
    return mapped;
  }
  ...
  // ---------- CRUD ----------
  async create(record: Omit<pasoUsuarioPermitido, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<pasoUsuarioPermitido, 'ID'>>) {
    await this.ensureIds()
