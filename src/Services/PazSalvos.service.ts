...
import type { PazSalvo, solicitados } from "../models/PazSalvo";
...
export class PazSalvosService {
  ...
  private toModel(item: any): PazSalvo {
    const f = item?.fields ?? {};
    let solicitados: solicitados[] = []
    if (item.fields.Solicitados) {
        try {
        solicitados = JSON.parse(item.fields.Solicitados) as solicitados[];
        } catch (e) {
        console.error("Error parseando Solicitados", e);
        solicitados = [];
        }
    }
  ...
  // ---------- CRUD ----------
  async create(record: Omit<PazSalvo, 'ID'>) {
    await this.ensureIds()
    const res = await this.graph.post<any>(
      `/sites/${this.siteId}/lists/${this.listId}/items`,
      { fields: {...record, Solicitados: JSON.stringify(record.Solicitados)} }
    );
    return this.toModel(res);
  }

  async update(id: string, changed: Partial<Omit<PazSalvo, 'ID'>>) {
        await this.ensureIds();
  ...
  async getAll(opts?: GetAllOpts): Promise<PageResult<PazSalvo>> {
    await this.ensureIds();
  ...
  async getByNextLink(nextLink: string): Promise<PageResult<PazSalvo>> {
    return this.fetchPage(nextLink, /*isAbsolute*/ true);
  }

  private async fetchPage(url: string, isAbsolute = false): Promise<PageResult<PazSalvo>> {
