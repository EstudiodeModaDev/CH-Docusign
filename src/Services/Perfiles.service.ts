...
export class PerfilesService {
  ...
  // ---------- mapping ----------
  private toModel(item: any): Perfil {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        Configuraciondeparametros: f.Configuraciondeparametros,
        Enviardocumentos: f.Enviardocumentos,
        Features: f.Features,
        Generarreportes: f.Generarreportes,
        Interfazgrafica: f.Interfazgrafica,
        Otorgarpermisos: f.Otorgarpermisos,
        Visualizardocumentosenviados: f.Visualizardocumentosenviados,
        Title: f.Title,
    };
  }

  // ---------- CRUD ----------
  async create(record: Omit<Perfil, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<Perfil, 'ID'>>) {
    await this.ensureIds();
