...
import type { permisos } from '../models/PazSalvo';
import { esc } from '../utils/text';

export class PermisosPazSalvosService {
  ...
  // ---------- mapping ----------
  private toModel(item: any): permisos {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        Title: f?.Title ?? '',
  ...
  // ---------- CRUD ----------
  async create(record: Omit<permisos, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<permisos, 'ID'>>) {
    await this.ensureIds();
