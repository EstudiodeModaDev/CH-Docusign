...
import type { renovar } from '../models/PazSalvo';
import { esc } from '../utils/text';

export class RenovarService {
  ...
  // ---------- mapping ----------
  private toModel(item: any): renovar {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        Title: f?.Title ?? '',
  ...
  // ---------- CRUD ----------
  async create(record: Omit<renovar, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<renovar, 'ID'>>) {
    await this.ensureIds();
