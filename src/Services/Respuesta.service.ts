...
export class RespuestaService {
  ...
  // ---------- mapping ----------
  private toModel(item: any): respuestas {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        Title: f?.Title ?? '',
        ...
        Area: f.Area,
        Correo: f.Correo,
        IdPazSalvo: f.IdPazSalvo,
        Respuesta: f.Respuesta,
    };
  }

  // ---------- CRUD ----------
  async create(record: Omit<respuestas, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<respuestas, 'ID'>>) {
    await this.ensureIds();
  ...
  async getAll(opts?: GetAllOpts): Promise<respuestas[]> {
    await this.ensureIds();
  ...
  // opcional: listar adjuntos ya subidos
