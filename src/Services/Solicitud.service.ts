...
export class solicitudService {
  ...
  constructor(
    ...
    listName = 'Actualizacion - Solicitud Cambio'     
  ) {
    this.graph = graph;
  ...
  // ---------- mapping ----------
  private toModel(item: any): solicitud {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        Title: f?.Title ?? '',
        Aprobador: f.Aprobador,
        comentarioAprobador: f.comentarioAprobador,
  ...
  // ---------- CRUD ----------
  async create(record: Omit<solicitud, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<solicitud, 'ID'>>) {
    await this.ensureIds();
  ...
  async getAll(opts?: GetAllOpts): Promise<solicitud[]> {
    await this.ensureIds();
