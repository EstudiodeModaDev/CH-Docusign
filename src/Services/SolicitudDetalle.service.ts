...
export class solicitudDetalleService {
  ...
  constructor(
    ...
    listName = 'Actualizacion - DetalleSolicitud'     
  ) {
    this.graph = graph;
  ...
  // ---------- mapping ----------
  private toModel(item: any): detalle {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        Title: f?.Title ?? '',
        EtiquetaCampo: f.EtiquetaCampo,
  ...
  // ---------- CRUD ----------
  async create(record: Omit<detalle, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<detalle, 'ID'>>) {
    await this.ensureIds();
  ...
  async getAll(opts?: GetAllOpts): Promise<detalle[]> {
    await this.ensureIds();
  ...
  async getBySolicitudId(solicitudId: string):Promise<detalle[]>{
    if(!solicitudId) return []

    const res = await this.getAll({filter:`fields/Title eq '${solicitudId}'`})
    return res
  }
}
