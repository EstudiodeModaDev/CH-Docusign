...
import type { dobleCampo } from '../models/Desplegables';
...
export class unidadNegocioService {
  ...
  // ---------- mapping ----------
  private toModel(item: any): dobleCampo {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        Title: f.Title,
        Abreviacion: f.Codigo
    };
  }

  // ---------- CRUD ----------
  async create(record: Omit<dobleCampo | withCode, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<dobleCampo | withCode, 'ID'>>) {
    await this.ensureIds();
