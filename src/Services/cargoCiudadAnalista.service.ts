...
import type { cargoCiudadAnalista } from "../models/Requisiciones/requisiciones";
import { esc } from "../utils/text";

export class cargoCiudadAnalistaService {
  ...
  constructor(
    ...
    listName = 'ANS - CargoCiudadAnalista'     
  ) {
    this.graph = graph;
  ...
  // ---------- mapping ----------
  private toModel(item: any): cargoCiudadAnalista {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        Cargo: f.Cargo,
        Ciudad: f.Ciudad,
        nombreAnalista: f.nombreAnalista,
        Title: f.Title,
    };
  }
  ...
  // ---------- CRUD ----------
  async create(record: Omit<cargoCiudadAnalista, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<cargoCiudadAnalista, 'ID'>>) {
    await this.ensureIds()
  ...
  async getAll(opts?: GetAllOpts): Promise<cargoCiudadAnalista[]> {
    await this.ensureIds()
