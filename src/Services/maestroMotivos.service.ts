...
import type { MaestroMotivos } from "../models/Requisiciones/requisiciones";
import { esc } from "../utils/text";

export class maestroMotivosService {
  ...
  constructor(
    ...
    listName = 'Requisiciones - MaestroMotivos'     
  ) {
    this.graph = graph;
  ...
  // ---------- mapping ----------
  private toModel(item: any): MaestroMotivos {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
  ...
  // ---------- CRUD ----------
  async create(record: Omit<MaestroMotivos, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<MaestroMotivos, 'ID'>>) {
    await this.ensureIds()
  ...
  async getAll(opts?: GetAllOpts): Promise<MaestroMotivos[]> {
    await this.ensureIds()
