...
export class MoverANSService {
  ...
  // ---------- mapping ----------
  private toModel(item: any): moverAns {
    const f = item?.fields ?? {};
    const mapped: moverAns = {
        Id: String(item?.id ?? ''),
        ANS: f.ANS,
  ...
  // ---------- CRUD ----------
  async create(record: Omit<moverAns, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<moverAns, 'ID'>>) {
    await this.ensureIds()
  ...
  async getAll(opts?: GetAllOpts): Promise<moverAns[]> {
    await this.ensureIds()
