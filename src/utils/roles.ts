...
import type { PerfilesService } from "../Services/Perfiles.service";
import type { UsuariosSPService } from "../Services/Usuarios.service";

/* =========================
   Tipos
   ========================= */
export type RoleDecision = | { role: string; source: "group"; matchedGroupId: string } | { role: string; source: "default" } | {role: string, permisos: any; source: "sp"};
...
/* =========================
   ...
   ========================= */
export async function getRoleFromSP(usuariosSvc: UsuariosSPService, email: string, PerfilSvc: PerfilesService): Promise<RoleDecision | null> {
  const safe = String(email).trim().toLowerCase();
  if (!safe) return null;
  ...
  const items: UsuariosSP[] = Array.isArray(resp) ? resp : resp?.items ?? [];
  const perfil: Perfil = await  PerfilSvc.get(items?.[0].IdPerfil)


  if (perfil) {
    return { 
      role: perfil.Title.trim(), 
      source: "sp", 
      permisos: {
        send: perfil.Enviardocumentos, 
        seguimiento: perfil.Visualizardocumentosenviados,
        reportes: perfil.Generarreportes,
        parametros: perfil.Configuraciondeparametros,
        permisos: perfil.Otorgarpermisos,
        interfaz: perfil.Interfazgrafica,
...
/* =========================
   ...
   ========================= */
export async function resolveUserRole({graph, usuariosSvc, email, groupRules = [], singleGroup, defaultRole = "Usuario", PerfilService}: {
  graph?: GraphRest;
  ...
  // 2) Lista SP
  try {
    const bySp = await getRoleFromSP(usuariosSvc, safeEmail, PerfilService);
    if (bySp) return bySp;
  } catch {  }
